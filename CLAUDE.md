...
Valgrind and `rchk` are no longer missing — both run in `native-checks.yaml`. `rchk` is informational until the package is at zero findings; flip `fail-on-findings: true` then, because after that the next finding is a regression. `gctorture(TRUE)` over both directions is still required by the definition of done below, because everything in that file runs after the fact.
