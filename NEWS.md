...
* The size of a frame is now capped. Because the result is rectangular, its
  ...
  The default is 50 million cells, roughly 400 MB of doubles — 100k rows by
  200 columns is 20 million, so a real tabular response has room to spare.
  `options(zujson.max_df_cells = )` changes it for the session, and
  `zujson_info()$max_df_cells` reports the limit actually in force.
